...
If you want, you can compute the characteristic polynomial for $M$ and check it has no real roots.
...
= An example showing a use for eigenvectors

Consider the following exercise
(too hard for 18.02 exams, but I want to show you how to solve it).
#exercise[
  Let $M$ be the matrix $mat(2,1;0,3)$.
  Calculate $M^100$.
]
This might look impossible at first glance without a computer.
...
#soln[
  ...
  and you know the outputs of $T$ on a basis, that tells you the outputs.

  So here's the idea: we'll find the eigenvectors for $M$,
  and pray they're independent (they are).
  ...
  $
  Thus encoding $M$ gives the answer: $M^100 = mat(2^100, 3^100-2^100; 0, 3^100)$.
]
...
Let $ A = mat(1,2;2,1), #h(1em) B = mat(1,2;1,-2), #h(1em) C = mat(-1,0,1;3,4,1;0,0,2). $
