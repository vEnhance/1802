...
#sample[
  ...
  Evaluate $integral bf(F) dot dif bf(r)$ and $integral bf(F) dot bf(n) dif s$.
]
...
#figure(
  table(
    ...
    [$cal(C)$ is a closed loop \ $==>$ Green],
      ...
      [Output $integral.double_(cal(R)) underbrace(2+2, "Div" = nabla dot bf(F)) dif A = 4pi$],
...
since $integral_(t=0)^(2 pi) sin 2 t$ is an integral over two full periods of the sine function.
...
#figure(
  table(
    ...
    [$cal(C)$ is a closed loop \ $==>$ Green],
      ...
      [Output $integral.double_(cal(R)) underbrace(2x+5, "Div" = nabla dot bf(F)) dif A = 5pi$],
    [Bare-hands definition \ Use parametrization],
...
so the answer is $integral.double_(cal(R)) (0-3) dif A = #boxed[$ -3 pi $]$.
...
#remark(title: [Polar coordinates is fine too for the flux one])[
  ...
  The inner integrals are $integral_(r=0)^1 r^2 dif r = 1 / 3$ and $integral_(r=0)^1 r dif r = 1 / 2$,
