...
$integral_(cal(C)) bf(F) dot dif bf(r)$ with bare-hands by using parametrization.
...
#sample[
  ...
  Compute $ integral_(cal(C)) 5 dif x. $
]
#soln[
...
#remark[
  ...
  Hence, $integral_(cal(C)) c dif x = c integral_(cal(C)) dif x$
...
There's a new symbol $integral.cont_(cal(C))$ on the left,
but it has the same meaning as $integral_(cal(C))$.
...
In other words:
#definition(title: [Definition of $integral.cont$])[
  $integral.cont_(cal(C))$ means "$integral_(cal(C))$ but with an extra optional reminder that $cal(C)$ is a loop".
...
So Green's theorem is then just saying that $integral.cont_(cal(C)) nabla f dif bf(r) = 0$
...
#sample[
  Evaluate the line integral $ integral.cont_(cal(C)) (y dif x - x dif y) $
...
#soln[
  ...
  $ op("Area")(cal(R)) = 1 / 2 dot upright("base") dot upright("height")
    = 1 / 2 dot 1 dot 1 = 1 / 2 . $
  Thus: $ integral.double_(cal(R)) - 2 dif A = - 2 dot 1 / 2 = #boxed[$ -1 $]. #qedhere $
]
...
#figure(
  ...
  caption: [Evaluation of $integral_(cal(C)) bf(F) dot dif bf(r)$ by "sealing"
...
#soln[
  ...
  $ underbrace(integral_(cal(C)) (2 y dif x + 3 x dif y), "what we want")
    + integral_(cal(C)_("lid")) (2 y dif x + 3 x dif y)
    &= integral.double_(cal(R)) ((partial q)/(partial x) - (partial p)/(partial y)) dif A \
  ...
  $ integral_(cal(C)_("lid")) (2 y dif x + 3 x dif y) = 0. $
...
#exer[Suggested by Ting-Wei Chao][
  ...
  Compute $integral_(cal(C)) bf(F) dot dif bf(r)$ this time using Green's Theorem.
] <exer-parabola-2>
