...
#figure(
  image("figures/chvar-trans.svg", width: 60%),
...
#memo(title: [Memorize: Change of variables])[
  ...
  $ integral.double_(cal(R)) f(x,y) dif x dif y = integral.double_(cal(S)) f(u,v) lr(|det J_(bf(T))|) dif u dif v $
  Alternatively, if it's easier to compute $J_(bf(T)^(-1))$, the following formula also works:
  $ integral.double_(cal(R)) f(x,y) dif x dif y = integral.double_(cal(S)) f(u,v)/lr(|det J_(bf(T)^(-1))|) dif u dif v $
]
